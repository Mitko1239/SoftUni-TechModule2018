...
namespace Fix
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                var matchDemonsDamage =
                    digetRegex.Matches(input[i]).Cast<Match>().Select(d => double.Parse(d.Value)).Sum();
                var matchDemonsHealth = letersRegex.Matches(input[i]).Cast<Match>()
