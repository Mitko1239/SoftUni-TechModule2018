    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while ((line = Console.ReadLine()) != "final")
            {
                ...
                char[] country1 = matches[0].Groups[2].Value.ToUpper().Reverse().ToArray();
                string country1Name = new string(country1);
                char[] country2 = matches[1].Groups[2].Value.ToUpper().Reverse().ToArray();
                string country2Name = new string(country2);
                int[] result = split[2].Split(":".ToCharArray(), StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();

                if (!leagueStandings.ContainsKey(country1Name))
                {
                    leagueStandings.Add(country1Name, new List<int>() {0, 0});
                }

                if (!leagueStandings.ContainsKey(country2Name))
                {
                    leagueStandings.Add(country2Name, new List<int>() {0, 0});
                }
